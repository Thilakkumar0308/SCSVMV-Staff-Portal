...
function send_reset_email($toEmail, $fullName, $resetLink) {
	...
	$headers = "From: no-reply@scsvmv.ac.in\r\nReply-To: no-reply@scsvmv.ac.in";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (empty($email)) {
		...
		// Always behave the same whether or not the user exists (to avoid user enumeration)
		if ($user = $result->fetch_assoc()) {
			...
			// Try sending email; if it fails, still show generic success to avoid info leakage
