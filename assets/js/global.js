...
info: "Showing *START* to *END* of *TOTAL* students",
infoEmpty: "Showing 0 to 0 of 0 students",
infoFiltered: "(filtered from *MAX* total students)",
...
if (!validateStudentForm(addStudentForm)) e.preventDefault();
});
}
...
if (editStudentForm) {
    editStudentForm.addEventListener('submit', e => {
        if (!validateStudentForm(editStudentForm)) e.preventDefault();
    });
}
...
if (addProfilePicture) {
    addProfilePicture.addEventListener('change', function() {
        previewStudentProfilePicture(this, 'add_profile_preview');
    });
}
...
if (editProfilePicture) {
    editProfilePicture.addEventListener('change', function() {
        previewStudentProfilePicture(this, 'edit_profile_preview');
    });
}
```
...
info: "Showing *START* to *END* of *TOTAL* departments",
infoEmpty: "Showing 0 to 0 of 0 departments",
infoFiltered: "(filtered from *MAX* total departments)",
...
/* -------------------------------
Student Form Validation (Added)
---------------------------------*/
function validateStudentForm(form) {
const requiredFields = form.querySelectorAll('[required]');
for (const field of requiredFields) {
if (!field.value.trim()) {
showAlert(`${field.name || 'A required field'} is missing`, 'danger');
field.focus();
return false;
}
}
